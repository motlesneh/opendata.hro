...
def markdown_extract(text, extract_length=500):
...
def format_icon(_format):
    ...
    if ('gml' in _format): return 'page_white_code'
    if ('kml' in _format): return 'page_white_world'
    if ('rss' in _format): return 'feed'
    if ('shape' in _format): return 'page_white_vector'
    if ('tms' in _format): return 'map'
    if ('wms' in _format): return 'world'
    if ('wfs' in _format): return 'vector'
    return 'page_white'
...
class Page(paginate.Page):
    ...
    def pager(self, *args, **kwargs):
        kwargs.update(
            ...
            symbol_previous=u'« zurück', symbol_next=u'vor »',
